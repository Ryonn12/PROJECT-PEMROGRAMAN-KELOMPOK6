...
int pinjam(int userId) {
  ...
  if (ditemukan) {
    // Mencatat data peminjaman (panggil fungsi untuk menulis ke pinjam.txt)
...
void recordPinjam(int userId, int bukuId) {
  // Membuka pinjam.txt untuk menambahkan data
  FILE* pinjamFile = fopen("pinjam.txt", "a");
  if (pinjamFile == NULL) {
